...
namespace disParity.CmdLine {
	public class Usage {
		...
		public static void Log(string msg) {
			...
			url = @"http://www.vilett.com/disParity/ping.php?cmd=" + msg + "&id=" + id.ToString() + "&firstRun=" + (firstRun ? "1" : "0") + "&ver=" + Version.VersionString;
		...
		public static void GetURL() {
			try {
				...
				if (latestVersion > 0 && latestVersion > currentVersion) {
					shutdownMsg = "Note: Version " + Encoding.ASCII.GetString(buf) + " of disParity is now available for download from www.vilett.com/disParity/\r\n";
				}
			} catch {
				return;
			}
		...
		static bool NoPing() {
			Object entry = Registry.GetValue("HKEY_CURRENT_USER\\Software\\disParity", "noping", 0);
