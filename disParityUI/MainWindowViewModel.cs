	...
	class MainWindowViewModel : NotifyPropertyChanged {
		...
		public MainWindowViewModel(Window owner) {
			...
			try {
				...
				App.GlobalConfig = config; // Add config to global Config.
				if (config.Exists) {
					config.Load();
					config.Validate();
				}
				config.HasLastHourlyUpdate = true;
		...
		public void Loaded() {
			...
			try {
				...
				MessageWindow.Show(
					owner,
					"FIPS requirement detected",
					"It appears that this system is configured to require FIPS certified cryptography. " + "Unfortunately disParity cannot run on a system with this requirement set.",
		...
		private void UpdateStartupMessage() {
			if (String.IsNullOrEmpty(config.ParityDir)) {
				...
				StartupMessage =
					"Add one or more drives to be backed up by pressing the 'Add Drive' button.\n\n" +
		...
		public void UpdateStatus() {
			...
			if (!scanRequired && !updateRequired) {
				...
				Status = String.Format("{1:N0} files ({0}) protected.  All drives up to date.", Utils.SmartSize(totalSize), totalFiles);
				return;
			}
			...
			if (config.MonitorDrives) {
				...
				if (nextAutoUpdate > DateTime.Now) {
					if ((nextAutoUpdate - DateTime.Now).Duration() < TimeSpan.FromHours(1)) {
						...
						Status = String.Format("{0}.  Next automatic update will occur {1} at {2}.", status, (nextAutoUpdate.Day == DateTime.Now.Day) ? "today" : "tomorrow", nextAutoUpdate.ToShortTimeString());
					}
				}
			} else {
		...
		public void Reset() {
			if (true != MessageWindow.Show(
				owner,
				"Confirm parity reset", "Are you sure you want to delete all of your parity data?",
				MessageWindowIcon.Error, MessageWindowButton.YesNo
			)) {
				return;
			}
			paritySet.Erase();
