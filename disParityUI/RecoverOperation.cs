...
namespace disParityUI {
	internal class RecoverOperation : CancellableOperation {
		...
		protected override void DoOperation() {
			...
			if (failures == 0) {
				string msg = String.Format("{0} file{1} successfully recovered!", successes, successes == 1 ? "" : "s");
			...
			Status = String.Format("{0} file{1} recovered ({2} failure{3})", successes, successes == 1 ? "" : "s", failures, failures == 1 ? "" : "s");
		}
