...
namespace disParityUI {
	public class OptionsDialogViewModel : NotifyPropertyChanged {
		...
		public void SetNewParityLocation(string path) {
			...
			foreach (DataDrive d in paritySet.Drives)
				if (String.Compare(root, Path.GetPathRoot(d.Root), true) == 0) {
					bool? result =
						MessageWindow.Show(Owner, "Duplicate drives detected!", "The selected parity location appears to be on one of your data drives.\n\n" +
			...
			if (!empty) {
				if (false == MessageWindow.Show(
					Owner,
					"Directory not empty",
					"This directory appears to contain existing parity data.  Are you sure you want to change to this location?",
		...
		private bool MoveParityData(string dest) {
			bool? result = MessageWindow.Show(
				Owner,
				"Move parity data?",
				"Would you like to move your existing parity data to the new location?",
				MessageWindowIcon.Question,
			...
			if (ParityDir != PARITY_NOT_SET) {
				if (Utils.PathsAreOnSameDrive(path, ParityDir)) {
					if (false == MessageWindow.Show(
						Owner,
						"Bad temp folder",
						"The path you selected appears to be on the same drive as your parity.\n\n" + "It is not recommended to have the temp folder on the same drive as your parity data.\n\n" + "Are you sure you want to set it to this location?",
						MessageWindowIcon.Caution,
						MessageWindowButton.YesNo
					)) {
						return;
					}
