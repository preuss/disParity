...
namespace disParity {
	...
	public class Config {
		...
		/// <summary>
		/// Default value for uploading Crash Log.
		/// </summary>
		const bool DEFAULT_UPLOAD_CRASH_LOG = false;
		...
		public void Reset() {
			...
			UploadCrashLog = DEFAULT_UPLOAD_CRASH_LOG;


			HasLastHourlyUpdate = false;
				...
				for (; ; ) {
					...
					if (reader.Name == "Options" && reader.IsStartElement()) {
						for ( ; ; ) {
							...
							} else if(reader.Name == "UploadCrashLog") {
								reader.Read();
								UploadCrashLog = ConfigUtil.ParseToBoolean(reader.Value);
								reader.Read();
							}
						}
		...
		public void Save() {
			...
			using (XmlWriter writer = XmlWriter.Create(filename, settings)) {
				...
				if (HasLastHourlyUpdate) {
					writer.WriteElementString("LastHourly", String.Format("{0}", LastHourlyUpdate.ToBinary()));
				}

				//if(UploadCrashLog != DEFAULT_UPLOAD_CRASH_LOG) {
					writer.WriteElementString("UploadCrashLog", UploadCrashLog.ToString().ToLower());
				//}
		...
		public Boolean HasLastHourlyUpdate { get; set; }
		...
		public bool UploadCrashLog { get; set; }
	}
