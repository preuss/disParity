...
namespace disParity {
	public class FileRecord {
		...
		internal FileRecord(FileInfo info, string path, string root) : this(root) {
			Initialize(info, Utils.MakeFullPath(path, info.Name), root);
		}

		internal FileRecord(string filePath, string root) : this(root) {
			FileInfo fi = new FileInfo(filePath);
			Initialize(fi, Utils.StripRoot(root, filePath), root);
		}

		internal FileRecord(FileInfo info, string root) : this(root) {
